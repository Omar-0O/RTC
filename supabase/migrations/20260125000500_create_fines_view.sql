...
-- Create a view to aggregate all fines from different participation tables
...
WHERE s.wore_vest = false AND s.status = 'approved'
...
SELECT
    ...
    c.name as source_name_ar, -- Caravans don't have name_ar in the types I saw, need to double check or just use name
...
SELECT
    ...
    e.name as source_name_ar, -- Events usually don't have name_ar?
...
-- formatting names:
-- I will assume name is enough if Ar is missing, or I handled it in select.
