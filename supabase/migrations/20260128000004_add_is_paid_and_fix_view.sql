-- Add is_paid column to activity_submissions for manual fines
ALTER TABLE public.activity_submissions ADD COLUMN IF NOT EXISTS is_paid boolean DEFAULT false;

-- Drop the view first because we are changing the column structure (adding source_id)
DROP VIEW IF EXISTS public.volunteer_fines_view;

-- Update the view to include source_id so we can uniquely identify automatic fines
-- This doesn't add is_paid to automatic fines yet, but prepares for it.
