...
BEGIN
  ...
  WHERE
    ...
    -- Exclude admins and other roles if needed, ensuring they don't appear in leaderboard
    AND NOT EXISTS (
  ...
  HAVING
    COALESCE(SUM(s.points_awarded), 0) > 0 -- Only show users with points in that period? Or show all? User request didn't specify, but typically leaderboard filters 0s or shows them at bottom. Let's show all but order by points.
    OR period_type = 'all_time' -- Show everyone for all time even if 0?
  ORDER BY
    total_points DESC,
