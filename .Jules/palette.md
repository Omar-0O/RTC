...
## 2024-05-23 - [Accessible Password Toggles]
**Learning:** Password visibility toggles often lack accessible names, making them confusing for screen reader users. Reusing translation keys ensures consistent labels across languages.
**Action:** Always verify icon-only buttons have an `aria-label` or `title` that reflects their current state (e.g., "Show password" vs "Hide password").
