...
export default function Reports() {
  ...
  const committeeData = useMemo(() => {
    const committeeStats = new Map<string, { count: number, points: number }>();
    profiles.forEach(p => {
      if (p.committee_id) {
        const stats = committeeStats.get(p.committee_id) || { count: 0, points: 0 };
        stats.count += 1;
        stats.points += (p.total_points || 0);
        committeeStats.set(p.committee_id, stats);
      }
    });

    return committees.map(committee => {
      const stats = committeeStats.get(committee.id) || { count: 0, points: 0 };
      return {
        name: language === 'ar' ? committee.name_ar : committee.name,
        volunteers: stats.count,
        points: stats.points,
      };
  ...
  const levelData = useMemo(() => [
    { name: t('level.under_follow_up'), value: volunteersByLevel.under_follow_up, color: '#64748b' }, // slate-500
    { name: t('level.project_responsible'), value: volunteersByLevel.project_responsible, color: '#3b82f6' }, // blue-500
    { name: t('level.responsible'), value: volunteersByLevel.responsible, color: '#9333ea' }, // purple-600
  ].filter(l => l.value > 0), [volunteersByLevel, t]);
  ...
  const activityStats = useMemo(() => activityTypes.map(activity => {
  ...
  }).filter(a => a.count > 0).sort((a, b) => b.count - a.count).slice(0, 5), [activityTypes, filteredSubmissions, language]);
