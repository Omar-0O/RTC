...
export default function Reports() {
  ...
  }).sort((a, b) => new Date(b.submitted_at).getTime() - new Date(a.submitted_at).getTime()), [submissions, getDateRange]);
  ...
  const totalVolunteers = useMemo(() => profiles.length, [profiles.length]);
  const totalApprovedActivities = useMemo(() => filteredSubmissions.filter(s => s.status === 'approved').length, [filteredSubmissions]);
  const totalSubmissions = useMemo(() => filteredSubmissions.length, [filteredSubmissions]);
