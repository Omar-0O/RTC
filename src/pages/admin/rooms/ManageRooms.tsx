        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50 hover:bg-muted/50">
              ...
              {canViewAllBranches && <TableHead className={`font-semibold ${isRTL ? "text-right" : "text-left"}`}>{isRTL ? "الفرع" : "Branch"}</TableHead>}
            ...
            {filteredRooms.map((room) => (
              <div key={room.id} className="border rounded-xl bg-card p-4 shadow-sm space-y-3 transition-all duration-200 hover:shadow-md">
                ...
                <div className="space-y-1.5 py-1">
                  ...
                  {canViewAllBranches && (
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-muted-foreground text-xs">{isRTL ? "الفرع" : "Branch"}</span>
                      <span className="font-medium">{isRTL ? (room.branches?.name_ar || '—') : (room.branches?.name || '—')}</span>
                    </div>
                  )}
                </div>
