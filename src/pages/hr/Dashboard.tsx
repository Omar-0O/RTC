...
export default function HRDashboard() {
    const { primaryRole, isLoading: authLoading } = useAuth();
    ...
    const isAuthorized = useMemo(() => {
        ...
        return ['hr', 'head_hr', 'admin', 'branch_admin', 'supervisor'].includes(primaryRole || '');
    }, [primaryRole, authLoading]);
    ...
    }, [activeBranch?.id, selectedMonth, isAuthorized]);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const nonAdminVolunteers = (profilesData || []).filter(
                (v: any) => !adminIds.includes(v.id)
            ) as unknown as Profile[];

            setVolunteers(nonAdminVolunteers);

            // 2. Fetch approved and pending submissions in selected month
            ...
            setAllSubmissions(submissionsData as unknown as Submission[]);
        } catch (error) {
    ...
    // Calculate aggregated volunteer stats
    const volunteerStatsList: VolunteerStats[] = useMemo(() => {
        return volunteers.map(volunteer => {
            const submissions = allSubmissions.filter(s => s.volunteer_id === volunteer.id);
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, isRTL ? 'تقرير الـ HR' : 'HR Report');
        XLSX.writeFile(wb, `HR_Dashboard_Report_${selectedMonth}.xlsx`);
